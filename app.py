...
print('[CONFIG] SMTP_HOST=' + SMTP_HOST + ' SMTP_USER=' + SMTP_USER + ' SMTP_PORT=' + str(SMTP_PORT))
...
.btn{display:inline-block;background:#00E5FF;color:#080B12;font-size:14px;font-weight:bold;padding:13px 32px;text-decoration:none;margin:16px 0}
...
<div class="wrap">
<div class="header"><div class="logo">XSQ HOST</div></div>
...
def send_email_async(to, subject, html):
    def do_send():
        if not SMTP_HOST or not SMTP_USER:
            print('[MAIL SKIP] No SMTP configured. To: ' + to)
            return
        try:
...
def mail_verify(to, username, token):
    ...
    html = base_email(
        ...
        '<p>Dein XSQHost Account wurde erstellt!</p>'
        '<div class="divider"></div>'
        '<a href="' + url + '" class="btn">E-MAIL BESTAETIGEN</a>'
        '<div class="divider"></div>'
        '<p style="font-size:12px">Link: <span style="color:#00E5FF">' + url + '</span></p>'
    )
    send_email_async(to, 'XSQHost – E-Mail bestaetigen', html)

def mail_server_created(to, username, srv):
    html = base_email(
        '<h1>Dein Server ist bereit!</h1>'
        '<p>Hey ' + username + ', dein FiveM Server wurde erstellt!</p>'
        '<div class="divider"></div>'
        '<p style="color:#00E5FF;font-size:12px">WINSCP / SFTP ZUGANGSDATEN</p>'
        '<div class="code-box">'
        '<div class="code-row"><span class="ck">Host:      </span><span class="cv">' + SFTP_HOST + '</span></div>'
        '<div class="code-row"><span class="ck">Port:      </span><span class="cv">' + str(srv['sftpPort']) + '</span></div>'
        ...
        '</div>'
        '<p style="color:#00E5FF;font-size:12px">MYSQL DATENBANK</p>'
        '<div class="code-box">'
    ...
    send_email_async(to, 'XSQHost – Server ' + srv['name'] + ' ist online!', html)
...
def mail_reset(to, username, token):
    ...
    html = base_email(
        ...
        '<div class="divider"></div>'
        '<a href="' + url + '" class="btn">PASSWORT ZURUECKSETZEN</a>'
        '<p style="font-size:12px">Dieser Link ist 1 Stunde gueltig.</p>'
    )
    send_email_async(to, 'XSQHost – Passwort zuruecksetzen', html)
...
def require_auth(f):
    ...
    def decorated(*args, **kwargs):
        ...
        try:
            ...
            return jsonify({'error': 'Token ungueltig oder abgelaufen'}), 401
...
def verify_email():
    ...
    if not user:
        return jsonify({'error': 'Token ungueltig'}), 404
    db = db_load()
    for u in db['users']:
        if u['id'] == user['id']:
            ...
            u['emailToken'] = None
    db_save(db)
    return jsonify({'message': 'E-Mail bestaetigt!'})
...
def create_server():
    try:
        ...
        if existing >= plan_config['maxServers']:
            return jsonify({'error': 'Dein ' + plan.upper() + ' Plan erlaubt maximal ' + str(plan_config['maxServers']) + ' Server.'}), 403
        ...
        print('[CREATE SERVER ERROR] ' + str(e))
        return jsonify({'error': 'Server erstellen fehlgeschlagen'}), 500
...
def update_status(sid):
    ...
    srv['status'] = status
    db_save(db)
    return jsonify({'message': 'Status geaendert'})
